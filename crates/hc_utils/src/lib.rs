    ...
    WasmError: From<<ET as EntryTypesHelper>::Error>,
{
    Ok(match register_update.original_action.entry_type() {
        EntryType::App(AppEntryDef {
            ...
            None => Err(guest_err(format!(
                "New entry is None meaning visibility is Private: {:?}",
                visibility
            )))?,
            Some(entry) => ET::deserialize_from_type(*zome_index, *entry_index, &entry)?
                .ok_or(guest_err("No entry type matched for:".to_string()))?,
        }),
        _ => None,
    })
    ...
    WasmError: From<<ET as EntryTypesHelper>::Error>,
{
    Ok(match register_delete.original_action.entry_type() {
        EntryType::App(AppEntryDef {
            ...
            None => Err(guest_err(format!(
                "Original entry is None meaning visibility is Private: {:?}",
                visibility
            )))?,
            Some(entry) => ET::deserialize_from_type(*zome_index, *entry_index, &entry)?
                .ok_or(guest_err("No entry type matched for:".to_string()))?,
...
pub fn zome_call_response_as_result(response: ZomeCallResponse) -> UtilResult<ExternIO> {
    Ok(match response {
        ...
        ZomeCallResponse::Unauthorized(zome_call_auth, cell_id, zome, func, agent) => Err(format!(
            "UnauthorizedError( {}, {}, {}, {}, {} )",
            zome_call_auth, cell_id, zome, func, agent
        )),
        ZomeCallResponse::NetworkError(message) => Err(format!("NetworkError( {} )", message)),
